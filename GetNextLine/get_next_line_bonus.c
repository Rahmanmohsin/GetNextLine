...
/*   Updated: 2024/04/04 05:05:46 by mohrahma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_queue	*queue[4096];
	char			*line;

	if (fd < 0 || fd > 4096 || BUFFER_SIZE < 1 || read(fd, NULL, 0) < 0)
	{
		free_all((queue + fd));
		return (NULL);
	}
	if (read_file(fd, (queue + fd)) == 0)
		return (free_all((queue + fd)), NULL);
	if (mem_size((queue + fd)) == 0)
		return (free_all((queue + fd)), NULL);
	line = get_l((queue + fd));
	return (line);
}
